  ...
  const [history, setHistory] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hello! Ask me about your connections and messages through all social handles",
    },
  ]);
  ...
  const handleClick = () => {
    ...
    fetch("/api/chat", {
      ...
      .then(async (res) => {
        ...
        setHistory((oldHistory) => [...oldHistory, resp]);
        setLoading(false);
      })
      .catch((err) => {
        alert(err);
  ...
  return (
    <main className="h-screen bg-black p-6 flex flex-col">
      <div className="flex flex-col gap-8 w-full items-center flex-grow max-h-full">
        <h1 className=" text-4xl text-transparent font-extralight bg-clip-text bg-gradient-to-b from-green-900 to-orange-900">
          hushhh.ai
        </h1>
        <form
          className="rounded-2xl border-purple-700 border-opacity-5  border lg:w-3/4 flex-grow flex flex-col bg-[url('/images/bg.webp')] bg-cover max-h-full overflow-clip"
          onSubmit={(e) => {
            e.preventDefault();
            handleClick();
          }}
                ...
                case "assistant":
                  return (
                    ...
                    >
                      ...
                      />
                      <div className="w-auto max-w-xl break-words bg-white rounded-b-xl rounded-tr-xl text-black p-6 shadow-[0_10px_40px_0px_rgba(0,0,0.15,0.15)]">
